...
export function derivePages(form: FormLike): FormPage[] {
  ...
  if (portal) return [{ ...portal, kind: "custom", label: portal.title }];
  ...
  return [
    ...
    ...(abstract ? [{ id: abstract.id, kind: "custom" as const, label: abstract.title, pageHeading: abstract.pageHeading, ...(abstract.description ? { description: abstract.description } : {}), fieldIds: abstract.fieldIds }] : []),
    ...(form.collectParticipants && participant ? [{ id: `${id}-participant`, kind: "system" as const, systemRole: "participant" as const, label: participant.title, pageHeading: participant.pageHeading, ...(participant.description ? { description: participant.description } : {}), fieldIds: participant.fieldIds }] : []),
...
export function assertValidPages(pages: FormPage[], formKind: string, collectParticipants: boolean) {
  ...
  const participant = pages.find((page) => page.systemRole === "participant");
  if (collectParticipants !== Boolean(participant)) throw new Error("The participant page must match participant collection.");
