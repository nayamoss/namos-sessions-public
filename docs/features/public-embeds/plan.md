...
competition brief. Naya's 2026-08-12 instruction to build embeds supersedes that cut. The current
...
- [x] T001: At implementation start, update `docs/features/INDEX.md` from `planned` to
...
- [x] T002: Keep feature documentation aligned with the canonical event-scoped
  `/events/:eventSlug/cms/embeds`, `/events/:eventSlug/cms/embeds/new`,
  `/events/:eventSlug/cms/embeds/:embedId`, and `/embed/:embedId` routes.
- [x] T003: Keep all four existing direct `/e/:eventSlug/:feed` pages from PR #63 working
...
- [x] T004: Add the `embeds` table and indexes from `design.md` to `convex/schema.ts`; this is a
  new-table-only migration with no backfill.
- [x] T005: Add `EmbedId`, `EmbedView`, `EmbedTheme`, `EmbedFieldOptions`, `Embed`, `EmbedWrite`,
  and `PublicEmbedView` types to `src/data/types.ts` exactly as specified in `design.md`.
- [x] T006: Expand `PublicEmbedsRepo` in `src/data/repo.ts` with organizer `list`, `getAdmin`,
  `preview`, `save`, `duplicate`, and `remove` operations plus public `getPublic`.
- [x] T007: Add the matching operation names and mappings to `src/data/transport.ts` and
  `src/data/convex/index.ts`.
- [x] T008: Update `src/data/airtable/index.ts` to fail closed for every `embeds.*` operation with
...
- [x] T009: Replace the single-purpose `convex/publicEmbeds.ts` module with the complete functions
...
- [x] T010: Validate name trimming/length, supported view/theme, hex color, field-option shape,
  track ownership, and event ownership before writing.
- [x] T011: Generate embed IDs only from Convex document IDs. Never place event IDs, speaker IDs,
  submission IDs, or agenda-item IDs in the public route or response.
- [x] T012: Build the public view projection server-side according to the saved embed view,
  selected tracks, and selected fields. Reject disabled embeds, unpublished events, and missing
  records with `null`.
- [x] T013: Keep public URLs restricted to `http:`/`https:` and resolve headshot storage IDs to
  fresh provider URLs on each public read.
- [x] T014: Add seed records in `convex/seed.ts`: enabled `Main event agenda` and disabled
  `Speaker gallery draft`, both attached to the seeded published event.

## Phase 4: Shared Embed Utilities

- [x] T015: Replace `src/lib/public-embed.ts` with helpers for `/embed/:embedId` URL generation,
...
- [x] T016: Generate a snippet containing a descriptive `title`, `loading="lazy"`, `width="100%"`,
  view-appropriate numeric `height`, `style="border:0;width:100%;"`, and
  `referrerpolicy="strict-origin-when-cross-origin"`.
- [x] T017: Do not add a new package. Use existing React, React Router, lucide-react, shadcn/Radix,
...
- **Route/location:** sidebar `CMS` section > `Embeds` > `/events/:eventSlug/cms/embeds`.
...
- **Behavior:** row/card click and Edit navigate to the active event's `/cms/embeds/:embedId`;
  Add navigates to the active event's `/cms/embeds/new`; Copy writes the generated snippet and shows toast `Embed code copied`;
...
- [x] T018: Add a `CMS` nav section to `src/components/AppLayout.tsx` with `Embeds` using the
  `Code2` icon. It must remain visible and labeled in expanded mode and have the canonical tooltip
  behavior in collapsed mode.
- [x] T019: Add the lazy event-scoped `/events/:eventSlug/cms/embeds` route in `src/App.tsx`.
- [x] T020: Build `EmbedsListPage` with every element and state above.
- [x] T021: Verify search, tabs, collapse group, copy, duplicate, toggle, delete/cancel, empty,
...
- **Routes/location:** `/events/:eventSlug/cms/embeds/new` and `/events/:eventSlug/cms/embeds/:embedId`.
...
- **Save success:** repository `save` returns ID; new route is replaced with
  the active event's `/cms/embeds/{id}`; toast `Embed saved`; dirty state clears.
...
- [x] T022: Add lazy editor routes to `src/App.tsx` and add exact route/title rows to
  `docs/DESIGN-SYSTEM.md`.
- [x] T023: Build `EmbedEditorPage` with every control, validation, dirty-state, save, and recovery
  behavior above using existing components only.
- [x] T024: Build `EmbedPreviewPanel` and wire every draft change to an immediate preview re-render.
- [x] T025: Add component-level tests for section visibility, required-field locking, validation,
...
- [x] T026: Refactor the current agenda/speaker markup into `EmbedRenderer`; add all five views and
  honor saved theme, primary color, filters, date/time formats, and field options.
- [x] T027: Add lazy `/embed/:embedId` route while retaining legacy `/e/:eventSlug/:feed` behavior.
- [x] T028: Add Netlify header configuration proving `/embed/*` is frameable; do not relax framing
  policy for organizer or speaker-portal routes.
- [x] T029: Browser-check each view at 375px and desktop width, including keyboard search/filter,
  inline expansion, empty, unavailable, loading, and error states.

## Phase 8: Security, Tests, and Verification

- [x] T030: Extend `src/test/public-embed.test.ts` for URL and snippet generation, field-map
  defaults, color validation, view labels, HTML attribute escaping, and opaque embed IDs.
- [x] T031: Add Convex tests proving: only published events; only enabled embeds; only published
...
- [x] T032: Add organizer CRUD tests for event isolation, track ownership, validation, disabled
  duplicate defaults, and deletion.
- [x] T033: Extend `src/test/data-adapter.contract.test.ts` only for the Convex-supported contract;
  explicitly assert the Airtable adapter fails closed rather than pretending parity.
- [x] T034: Run `npm run typecheck`, `npm run test`, `npm run build`, `npm run lint`,
  `git diff --check`, and the repository's secret scan.
- [x] T035: Drive every step in `USER_JOURNEY.md` in the browser from sidebar entry through code
  copied into a blank external HTML fixture. Capture desktop and 375px evidence.
- [x] T036: Verify a published agenda change and accepted-speaker profile change appear on the next
  iframe load without regenerating code.
- [x] T037: Update `docs/features/INDEX.md` to `done` only after the full browser journey and public
...
- [x] All acceptance criteria in `requirements.md` are met.
- [x] Organizer reaches CMS > Embeds from the sidebar without typing a route.
- [x] Create, edit, save, preview, copy, enable/disable, duplicate, and delete work visibly.
- [x] Configuration and state persist after refresh and a new browser session.
- [x] All five view types render and honor supported filters/fields.
- [x] Generated iframe works in a blank external HTML page at desktop and 375px.
- [x] Public projection and route expose no private records or fields.
- [x] Disabled/deleted embeds and unpublished events reveal no event metadata.
- [x] Public data updates on the next load without code regeneration.
- [x] Empty, loading, validation, API failure, clipboard failure, and unavailable states are proven.
- [x] Public route remains lazy-loaded and intentionally frameable without making admin routes
  frameable.
- [x] No regression to the four legacy `/e/:eventSlug/:feed` links delivered by PR #63.
- [x] Docs and `docs/features/INDEX.md` are updated in the same implementation commit.
