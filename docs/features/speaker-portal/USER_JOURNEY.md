...
and use the explicit **Demo impersonation** picker. That is a demo and support path, not the speaker
...
1. **Resolve the speaker identity.** The user sees the shared dashboard shell and speaker navigation.
   ...
   does not see the demo picker and cannot switch to another speaker.
...
| Verified email does not match a speaker | No owned profile; organizer accounts may see the explicitly labelled demo picker | A real speaker must correct/verify the Clerk email or ask the organizer to correct the speaker record | Implemented fail-closed; recovery copy is incomplete for real speakers |
