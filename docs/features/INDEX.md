...
| 37 | [evaluation-scorecards](./evaluation-scorecards/plan.md) | `done` | 8-12h | #4 | — | Weighted multi-criterion scorecards on an evaluation plan, replacing the single bare `evaluations.score` number. Chairs define criteria, reviewers score each one, and the organizer grid sorts by the weighted total; legacy single-score reviews still render and a plan with zero criteria is unchanged. Shipped upstream as webapp #56. |
