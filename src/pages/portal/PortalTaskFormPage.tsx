...
export default function PortalTaskFormPage({ formId }: { formId: string }) {
  ...
  if (!eventId || !selectedSpeaker) return <section className="rounded-lg bg-card p-6"><p className="font-semibold">Choose a speaker to continue</p></section>;
  return <div className="space-y-6"><div><Link to="/portal/tasks" className="text-sm underline">Back to tasks</Link><h1 className="mt-3 text-2xl font-bold">{form?.title ?? "Portal form"}</h1></div>{error && <p role="alert" className="text-sm text-destructive">{error}</p>}{message && <p role="status" className="rounded-md bg-muted p-3 text-sm">{message}</p>}{loading ? <SkeletonList rows={3} label="Loading form…" /> : form && <section className="space-y-5 rounded-lg bg-card p-6"><div><h2 className="font-semibold">{form.sectionTitle}</h2>{form.description && <p className="mt-1 text-sm text-muted-foreground">{form.description.replace(/<[^>]*>/g, "")}</p>}</div><DynamicFormRenderer fields={form.fields} values={answers} onChange={(id, value) => setAnswers(current => ({ ...current, [id]: value }))} /><div className="flex justify-end"><Button variant="accent" disabled={saving} onClick={() => void submit()}>{saving ? "Saving…" : "Save form"}</Button></div></section>}</div>;
}
