...
export default function PortalTaskFormPage({ formId }: { formId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const result = await repo.portalForms.get({
        eventId,
        formId,
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "Could not load this portal form.",
      );
    } finally {
      setLoading(false);
    }
  }, [eventId, formId, repo, selectedSpeaker]);
  useEffect(() => {
    void load();
  }, [load]);
  ...
  const submit = async () => {
    if (!eventId || !selectedSpeaker) return;
    setSaving(true);
    setError(undefined);
    try {
      await repo.portalForms.submit({
        eventId,
        formId,
        speakerId: selectedSpeaker.id,
        taskId,
      ...
      setMessage(taskId ? "Form saved and task completed." : "Form saved.");
    } catch (cause) {
      setError(
        cause instanceof Error ? cause.message : "Could not save this form.",
      );
    } finally {
      setSaving(false);
    }
  ...
  if (!eventId || !selectedSpeaker)
    return (
      <section className={cardSurfaceClasses("default", "p-6")}>
        <p className="font-semibold">Choose a speaker to continue</p>
      </section>
    );
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <SkeletonList rows={3} label="Loading form…" />
      ) : (
        form && (
          <section className={cardSurfaceClasses("default", "space-y-5 p-6")}>
            <div>
              <h2 className="font-semibold">{form.sectionTitle}</h2>
              {form.description && (
                <p className="mt-1 text-sm text-muted-foreground">
                  {stripHtmlTags(form.description)}
                </p>
              )}
            </div>
            <DynamicFormRenderer
