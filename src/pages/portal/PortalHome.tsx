...
function PortalFormRoute() {
  const { formId } = useParams();
  return formId ? <PortalTaskFormPage formId={formId} /> : null;
}
    ...
    <PortalIdentityProvider>
      <PortalLayout>
        <Routes>
          ...
          <Route path="submissions/new" element={<PortalNewSubmission />} />
          <Route
          ...
          <Route path="tasks" element={<Navigate to="/portal" replace />} />
          <Route path="forms/:formId" element={<PortalFormRoute />} />
        </Routes>
      </PortalLayout>
    </PortalIdentityProvider>
  );
