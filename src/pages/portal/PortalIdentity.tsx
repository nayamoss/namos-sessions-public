...
type PortalIdentity = {
  ...
  /**
   * True once the signed-in Clerk account resolved to a speaker by email. The demo speaker
   * picker in PortalLayout stays hidden while this is true — it exists only as a fallback for
   * accounts that don't match a speaker record yet.
   */
...
export function PortalIdentityProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        // A matched Clerk account sees only its own speaker record. Organizer accounts that
        // are not themselves speakers retain the explicit demo picker as a fallback.
        ...
        // not a credential. Anything else falls through to the dropdown.
  ...
  const selectSpeaker = useCallback((speakerId: string) => {
    // The signed-in identity is authoritative once resolved; the picker is a fallback only.
    if (identityLockedByClerk || !eventId || !speakers.some(speaker => speaker.id === speakerId)) return;
    window.localStorage.setItem(storageKey(eventId), speakerId);
    setSelectedSpeakerId(speakerId);
  }, [eventId, speakers, identityLockedByClerk]);

  const selectedSpeaker = speakers.find(speaker => speaker.id === selectedSpeakerId);
  return <PortalIdentityContext.Provider value={{ eventId, eventName, event: eventData, speakers, selectedSpeaker, loading, error, selectSpeaker, identityLockedByClerk, handoffMismatch }}>{children}</PortalIdentityContext.Provider>;
}
