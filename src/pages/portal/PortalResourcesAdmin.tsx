...
export default function PortalResourcesAdmin() {
  ...
  const load = useCallback(async () => {
    ...
    setError(undefined);
    try {
      setPages(await repo.portalResources.listAdmin({ eventId: event.id }));
    } catch (cause) {
      setError(
        cause instanceof Error
          ? cause.message
          : "Could not load portal resources.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const save = async () => {
    ...
    setError(undefined);
    try {
      const id = await repo.portalResources.save({
        eventId: event.id,
        id: selected?.id,
        ...draft,
      });
  ...
  const remove = async () => {
    ...
    setError(undefined);
    try {
      await repo.portalResources.remove({ eventId: event.id, id: selected.id });
      close();
      await load();
    } catch (cause) {
      setError(
  ...
  const move = async (id: string, direction: -1 | 1) => {
    ...
    setPages(reordered);
    try {
      await repo.portalResources.reorder({
        eventId: event.id,
        ids: reordered.map((page) => page.id),
      });
    } catch (cause) {
      setError(
        cause instanceof Error ? cause.message : "Could not reorder resources.",
      );
      await load();
    }
  };
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <RichTextEditor
              ...
              placeholder="Add arrival details, venue guidance, contacts, or speaker policies…"
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="resource-status">Visibility</Label>
            <Select
              value={draft.status}
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="draft">Draft — organizers only</SelectItem>
                <SelectItem value="published">
                  Published — visible to speakers
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          {draft.bodyHtml && (
            ...
            >
              ...
              <RichText html={draft.bodyHtml} className="text-sm" />
            </section>
          )}
          {error && (
  ...
  return (
    <AppLayout title="Portal resources" detail={detail}>
      <div className="space-y-4">
        ...
        <section
          className={cardSurfaceClasses("default", "overflow-hidden")}
          aria-label="Portal resources"
        >
          {loading ? (
            ...
            pages.map((page, index) => (
              <div
                key={page.id}
                className="flex flex-wrap items-center gap-3 border-b border-border/60 p-4 last:border-b-0"
              >
                <Button
                  type="button"
                  variant="ghost"
            ...
            <EmptyState
              icon={BookOpen}
              title="Create the speaker handbook"
              message="Publish arrival details, venue guidance, contacts, and policies in one shared resource area."
              action={
                <Button
                  type="button"
                  variant="accent"
