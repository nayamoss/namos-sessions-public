...
export default function ApiKeys() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setKeys(await repo.apiKeys.list({ eventId: event.id }));
      setError(undefined);
    } catch (cause) {
      setError(
  ...
  const generate = async () => {
    setAttempted(true);
    if (!label.trim()) {
      setError("Give this key a label.");
      return;
    }
    if (!scopes.length) return;
    setSaving(true);
    try {
      const result = await repo.apiKeys.generate({
        eventId: event.id,
        label,
        scopes,
      });
  ...
  const revoke = async () => {
    ...
    setSaving(true);
    try {
      await repo.apiKeys.revoke({ eventId: event.id, id: revokeKey.id });
  ...
  const detail = paneOpen ? (
    <DetailPane
      title={rawKey ? "Store your key" : "Generate key"}
      onClose={closePane}
    >
      {rawKey ? (
        <div className="space-y-4">
          ...
          <Button type="button" variant="accent" size="sm" onClick={closePane}>
            Done
          </Button>
        </div>
      ) : (
        <div className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="api-key-label">What’s this token for?</Label>
            <Input
              id="api-key-label"
              autoFocus
              maxLength={80}
          ...
          <div className="flex justify-end gap-2">
            <Button type="button" variant="ghost" size="sm" onClick={closePane}>
              Cancel
            </Button>
            <Button
              type="button"
    ...
    <>
      <div className="space-y-4">
        <p className="text-base text-muted-foreground">
          Create scoped tokens for the Namos Sessions API.
        </p>
        <ContentToolbar
          ariaLabel="API token actions"
          primaryAction={
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            message="Create a scoped token when an integration is ready."
            action={
              <Button
                variant="accent"
                size="sm"
          ...
          <div className="space-y-3">
            {keys.map((key) => (
              <article
                key={key.id}
                className={cardSurfaceClasses("default", "bg-muted/60 p-4")}
              >
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <h2 className="font-medium">{key.label}</h2>
                    <code className="mt-1 block truncate font-mono text-sm text-muted-foreground">
                      {key.keyPrefix}••••••••
                    </code>
        ...
        <section className={cardSurfaceClasses("default", "bg-muted/60")}>
          <button
            type="button"
          ...
          {auditOpen && (
            <div className="px-4 pb-4">
              <ApiAuditLogTable eventId={event.id} />
            </div>
          )}
        </section>
        <AlertDialog
          ...
          onOpenChange={(open) => {
            if (!open) setRevokeKey(undefined);
          }}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Revoke {revokeKey?.label}?</AlertDialogTitle>
              <AlertDialogDescription>
                This immediately stops the key from working. This can’t be
                undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
