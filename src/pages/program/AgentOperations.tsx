...
import type { AgentActionProposal, AgentProposalId, AgentRun, AgentRunDetail, AgentRunId, AgentSuggestion } from "@/data/types";

const fallbackSuggestions = ["Check whether this event is ready to publish", "Find accepted speakers who still need attention", "Review failed communications and overdue tasks"];
...
export default function AgentOperations() {
  ...
  const suggestions = useRepoQuery<AgentSuggestion[]>("agentRuns.suggestions", access.data ? { eventId: event.id } : "skip");
  ...
  const approveProposal = (proposal: AgentActionProposal) => decide(
    proposal.id,
  ...
  const inspector = selected ? <AgentRunInspector run={selected.run} proposals={selected.proposals} showRunMeta={false} decisionPendingId={decisionPendingId} error={error} onApprove={approveProposal} onReject={(proposalId) => decide(proposalId, () => repo.agentRuns.rejectProposal({ eventId: event.id, proposalId })).then(() => undefined)} onCancel={() => repo.agentRuns.cancel({ eventId: event.id, runId: selected.run.id })} onRetry={() => repo.agentRuns.retry({ eventId: event.id, runId: selected.run.id })} /> : undefined;
