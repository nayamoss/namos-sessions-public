...
export default function AgentOperations() {
  ...
  const inspector = selected ? <AgentRunInspector run={selected.run} proposals={selected.proposals} decisionPendingId={decisionPendingId} error={error} onApprove={(proposalId, hash) => decide(proposalId, () => repo.agentRuns.approveTaskProposal({ eventId: event.id, proposalId, expectedPayloadHash: hash })).then(() => undefined)} onReject={(proposalId) => decide(proposalId, () => repo.agentRuns.rejectProposal({ eventId: event.id, proposalId })).then(() => undefined)} onCancel={() => repo.agentRuns.cancel({ eventId: event.id, runId: selected.run.id })} onRetry={() => repo.agentRuns.retry({ eventId: event.id, runId: selected.run.id })} /> : undefined;
  ...
  return <AppLayout title="Operations Agent" detail={inspector}><div className="flex min-h-[calc(100vh-10rem)] flex-col gap-4"><ContentToolbar ariaLabel="Agent run controls" utilities={<><AgentHistoryPopover runs={history.data ?? []} selectedRunId={selectedRunId ?? undefined} onSelect={(runId) => setParams({ run: runId })} isLoading={history.isLoading} /><Button variant="outline" size="sm" onClick={() => setParams({})}><Plus className="h-4 w-4" />New run</Button></>} /><div className="min-h-0 flex-1 overflow-y-auto"><AgentTimeline events={selected?.events ?? []} isLoading={Boolean(selectedRunId) && detail.isLoading} />{!selectedRunId && <div className="mt-4 flex flex-wrap justify-center gap-2">{suggestions.map((suggestion) => <Button key={suggestion} variant="outline" size="sm" onClick={() => setValue(suggestion)}><Bot className="h-4 w-4" />{suggestion}</Button>)}</div>}{detail.error && <p role="alert" className="text-sm text-destructive">{detail.error.message}</p>}</div><AgentComposer value={value} onChange={setValue} onSubmit={() => void submit()} mode={replyMode ? "reply" : "new"} disabled={submitting || Boolean(selected && !replyMode && ["queued", "running"].includes(selected.run.status))} error={error} /><p className="sr-only" aria-live="polite">{selected ? `Run status: ${selected.run.status.replace(/_/g, " ")}` : "Ready for a new run"}</p></div></AppLayout>;
}
