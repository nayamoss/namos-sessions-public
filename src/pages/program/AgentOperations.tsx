...
const fallbackSuggestions = [
  "Check whether this event is ready to publish",
  "Find accepted speakers who still need attention",
  "Review failed communications and overdue tasks",
];
const newKey = () =>
...
export default function AgentOperations() {
  ...
  const access = useRepoQuery<boolean>("agentRuns.canUse", {
    eventId: event.id,
  });
  ...
  const running = Boolean(
    selected &&
    !replyMode &&
    ["queued", "running"].includes(selected.run.status),
  );
  ...
  useEffect(() => {
    const handleToggle = () => {
      if (!busy) setVoiceOpen((open) => !open);
    };
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(
        cause instanceof Error ? cause.message : "Could not submit this run.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const inspector = selected ? (
    <AgentRunInspector
      ...
      onReject={(proposalId) =>
        decide(proposalId, () =>
          repo.agentRuns.rejectProposal({ eventId: event.id, proposalId }),
        ).then(() => undefined)
      }
      onCancel={() =>
        repo.agentRuns.cancel({ eventId: event.id, runId: selected.run.id })
      }
      onRetry={() =>
        repo.agentRuns.retry({ eventId: event.id, runId: selected.run.id })
      }
    />
  ) : undefined;
  ...
  if (access.error?.message.includes("Convex backend"))
    return (
      <AppLayout title="Operations Agent">
    ...
    return (
      <AppLayout title="Operations Agent">
        <div className="space-y-3">
          <h2 className="text-lg font-semibold">Organizer access required</h2>
          <p className="text-sm text-muted-foreground">
            Only an event organizer can use the Operations Agent.
          </p>
        </div>
      </AppLayout>
    );
  ...
  return (
    <AppLayout title="Operations Agent" detail={inspector}>
      <div className="flex min-h-[calc(100vh-10rem)] flex-col gap-4">
        <ContentToolbar
          ariaLabel="Agent run controls"
          utilities={
            <>
              <AgentHistoryPopover
                runs={history.data ?? []}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <AgentTimeline
            ...
            isLoading={Boolean(selectedRunId) && detail.isLoading}
          />
          {!selectedRunId && (
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              {groundedSuggestions.length
                ? groundedSuggestions.map((suggestion) => (
                    <Button
                      key={suggestion.id}
                      variant="outline"
                      size="sm"
                      title={`Evidence: ${suggestion.evidenceCount} · ${suggestion.sourcePath}`}
                      onClick={() => setValue(suggestion.objective)}
                    >
        ...
        <p className="sr-only" aria-live="polite">
          {selected
            ? `Run status: ${selected.run.status.replace(/_/g, " ")}`
            : "Ready for a new run"}
        </p>
      </div>
      {voiceOpen && (
