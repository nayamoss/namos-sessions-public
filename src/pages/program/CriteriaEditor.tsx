    ...
    <div className="space-y-3">
      ...
      {criteria.length === 0 ? (
        ...
        >
          ...
          <p className="min-w-0 flex-1 font-medium">No criteria yet</p>
          <Button
            variant="accent"
            size="sm"
              ...
              <div className="grid items-center gap-2 md:grid-cols-[minmax(0,1fr)_8rem_5.5rem_5.5rem_auto_auto]">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    update(
                      criterion.id,
                      value === "text"
                        ? { type: "text", max: undefined, weight: undefined }
                        : {
                            type: "number",
                            max: criterion.max ?? scoringScaleMax,
                  ...
                  disabled={disabled}
                >
                  <SelectTrigger aria-label="Criterion type">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="number">Score</SelectItem>
                ...
                {criterion.type === "number" ? (
                  <Input
                    ...
                    aria-label={`Maximum for ${criterion.label || "criterion"}`}
                    disabled={disabled}
                  />
                ) : (
                ...
                <label className="flex items-center gap-2 text-sm text-muted-foreground">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      update(criterion.id, { required: checked === true })
                    }
                    aria-label={`Require ${criterion.label || "criterion"}`}
                    disabled={disabled}
                  />
                  Required
                </label>
