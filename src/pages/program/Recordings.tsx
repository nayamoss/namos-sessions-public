...
import { ArrowUpDown, CheckCircle2, CircleAlert, Film, Link2, ListFilter, Loader2, Search, Upload, Video, X } from "lucide-react";
...
function FilterMenuSub({ label, value, values, onChange }: { label: string; value: string; values: Array<[string, string]>; onChange: (value: string) => void }) {
  const selectedLabel = values.find(([candidate]) => candidate === value)?.[1] ?? label;
  return <DropdownMenuSub><DropdownMenuSubTrigger><span>{label}</span><span className="ml-auto max-w-28 truncate px-2 text-xs font-normal text-muted-foreground">{selectedLabel}</span></DropdownMenuSubTrigger><DropdownMenuSubContent className="max-h-72 w-56 overflow-y-auto"><DropdownMenuLabel>{label}</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuRadioGroup value={value} onValueChange={onChange}>{values.map(([candidate, candidateLabel]) => <DropdownMenuRadioItem key={candidate} value={candidate}>{candidateLabel}</DropdownMenuRadioItem>)}</DropdownMenuRadioGroup></DropdownMenuSubContent></DropdownMenuSub>;
}
...
export default function Recordings() {
  ...
  const clearFilters = () => {
    setFilter("all"); setSourceFilter("all"); setRoomFilter("all"); setTrackFilter("all"); setDayFilter("all");
    setParams(current => { const next = new URLSearchParams(current); ["filter", "source", "room", "track", "day"].forEach(key => next.delete(key)); return next; });
  };
  ...
  const activeFilterCount = [filter !== "all", sourceFilter !== "all", roomFilter !== "all", trackFilter !== "all", dayFilter !== "all"].filter(Boolean).length;
  const statusFilters: Array<[Filter, string]> = [["all", "Any status"], ["missing", "Missing"], ["draft", "Ready drafts"], ["published", "Published"], ["replacement", "Replacements"], ["unavailable", "Unavailable"]];
  return <AppLayout title="Recordings" detail={selectedRow ? <RecordingDetailPane eventId={event.id} agendaItemId={selectedRow.id} onClose={() => select()} onChanged={load} /> : undefined}>
    <div className="space-y-4">
      <ContentToolbar ariaLabel="Recording controls" search={<div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => { const value = event.target.value; setQuery(value); setParams(current => { const next = new URLSearchParams(current); if (value.trim()) next.set("q", value); else next.delete("q"); return next; }); }} className="h-9 pl-9" placeholder="Search sessions or speakers" aria-label="Search recordings" /></div>} utilities={<><DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" variant={activeFilterCount > 0 ? "subtle" : "outline"} className="relative" title={`Filters${activeFilterCount > 0 ? `, ${activeFilterCount} active` : ""}`} aria-label={`Filters${activeFilterCount > 0 ? `, ${activeFilterCount} active` : ""}`}><ListFilter className="h-3.5 w-3.5" />{activeFilterCount > 0 && <span className="absolute -right-1 -top-1 rounded-full bg-foreground/10 px-1 text-[10px] tabular-nums">{activeFilterCount}</span>}</Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-64"><DropdownMenuLabel>Filter recordings</DropdownMenuLabel><DropdownMenuSeparator /><FilterMenuSub label="Status" value={filter} values={statusFilters} onChange={value => applyFilter(value as Filter)} /><FilterMenuSub label="Source" value={sourceFilter} values={[["all", "Any source"], ["upload", "Direct upload"], ["asset", "Event asset"], ["hosted", "Hosted link"]]} onChange={value => { setSourceFilter(value as SourceFilter); applyQueryFilter("source", value); }} /><FilterMenuSub label="Room" value={roomFilter} values={[["all", "All rooms"], ...rooms]} onChange={value => { setRoomFilter(value); applyQueryFilter("room", value); }} /><FilterMenuSub label="Track" value={trackFilter} values={[["all", "All tracks"], ...tracks]} onChange={value => { setTrackFilter(value); applyQueryFilter("track", value); }} /><FilterMenuSub label="Day" value={dayFilter} values={[["all", "All days"], ...days.map(day => [day, day] as [string, string])]} onChange={value => { setDayFilter(value); applyQueryFilter("day", value); }} /><DropdownMenuSeparator /><DropdownMenuItem disabled={activeFilterCount === 0} onSelect={clearFilters}>Clear filters</DropdownMenuItem></DropdownMenuContent></DropdownMenu><DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" variant="outline" title={`Sort: ${sort === "schedule_asc" ? "Schedule, earliest first" : "Schedule, latest first"}`} aria-label={`Sort: ${sort === "schedule_asc" ? "Schedule, earliest first" : "Schedule, latest first"}`}><ArrowUpDown className="h-3.5 w-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-52"><DropdownMenuLabel>Sort by schedule</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuRadioGroup value={sort} onValueChange={value => { const nextSort = value as RecordingManagerSort; setSort(nextSort); setParams(current => { const next = new URLSearchParams(current); if (nextSort === "schedule_asc") next.delete("sort"); else next.set("sort", nextSort); return next; }); }}><DropdownMenuRadioItem value="schedule_asc">Earliest first</DropdownMenuRadioItem><DropdownMenuRadioItem value="schedule_desc">Latest first</DropdownMenuRadioItem></DropdownMenuRadioGroup></DropdownMenuContent></DropdownMenu></>} primaryAction={<Button size="sm" onClick={() => { const target = rows.find(row => !row.recording) ?? rows[0]; if (target) select(target.id); }}>Add recording</Button>} />
