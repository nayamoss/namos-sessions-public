...
export type WriteOperation =
  ...
  | "taskTemplates.ensureStarters"
  | "taskTemplates.applyToSubmission"
  | "taskTemplates.applyToSpeaker"
...
function trackWriteSuccess(
  ...
  switch (operation) {
    case "events.save":
      track(input.id ? "event_updated" : "event_created", {
        event_status: input.status as
    ...
    case "forms.save":
      track("form_saved", {
        mode: input.id ? "updated" : "created",
        form_kind: input.kind as never,
      });
      break;
    case "forms.setStatus":
      track("form_status_updated", { form_status: input.status as never });
      break;
    case "forms.duplicate":
    case "forms.createFromTemplate":
      track("form_duplicated", {});
      break;
    case "forms.remove":
      track("record_removed", { record_type: "form" });
      break;
    case "submissions.createAdmin": {
      ...
      track("submission_created", {
        source: "organizer",
        submission_status:
          status === "accepted" || status === "declined" || status === "draft"
            ? status
            : "pending",
      });
      break;
    ...
    case "submissions.saveDraft":
      track("submission_created", {
        source: "organizer",
        submission_status:
          operation === "submissions.saveDraft" ? "draft" : "pending",
      });
      break;
    case "submissions.decide":
    case "submissions.setStatus":
      track("submission_status_updated", {
        submission_status: normalizedSubmissionStatus(input.status),
      });
      break;
    case "publicForms.submit":
      track("public_submission_completed", {
        participant_count: Array.isArray(object(input.input).participants)
          ? (object(input.input).participants as unknown[]).length
          : 0,
      });
      break;
    case "speakers.create":
      track("speaker_created", {});
      break;
    case "speakers.bulkImport":
      track("speakers_imported", {
        imported_count: Number(result.importedSpeakers || 0),
        skipped_count: Array.isArray(result.skipped)
          ? result.skipped.length
          : 0,
      });
      break;
    case "speakers.setConfirmationStatus":
      track("speaker_confirmation_updated", {
        confirmation_status: input.status as never,
      });
      break;
    case "speakers.updateProfile":
      track("speaker_profile_updated", { has_bio: Boolean(input.bio) });
      break;
    case "speakers.saveHeadshot":
      track("speaker_profile_updated", { has_headshot: true });
      break;
    case "speakers.documents.remove":
      track("record_removed", { record_type: "speaker_document" });
      break;
    case "sponsors.create":
      track("sponsor_saved", {
        mode: "created",
        sponsor_status: input.status as never,
      });
      break;
    ...
    case "sponsors.remove":
      track("record_removed", { record_type: "sponsor" });
      break;
    case "evaluations.plans.save":
      track("review_plan_saved", {
        mode: input.id ? "updated" : "created",
        round_count:
          typeof input.rounds === "number" ? input.rounds : undefined,
        anonymized:
          typeof input.anonymized === "boolean" ? input.anonymized : undefined,
      });
      break;
    case "evaluations.assignments.assign":
      track("review_assignments_created", { created_count: 1 });
      break;
    case "evaluations.assignments.assignByFilter":
      track("review_assignments_created", {
        created_count: Number(result.created || 0),
        skipped_count: Number(result.skipped || 0),
      });
      break;
    case "evaluations.save":
      track("review_completed", {
        has_scorecard: Array.isArray(input.criteriaScores),
      });
      break;
    case "evaluations.sendReviewerReminders":
      track("reviewer_reminders_sent", {
        sent_count: Number(result.sent || 0),
        failed_count: Number(result.failed || 0),
      });
      break;
    case "agenda.save":
      track("agenda_session_saved", {
        mode: input.id ? "updated" : "created",
        published:
      ...
      break;
    case "agenda.publishSchedule":
      track("agenda_published", {
        published_count: typeof result === "number" ? result : undefined,
      });
      break;
    case "agenda.remove":
      track("record_removed", { record_type: "agenda_session" });
      break;
    case "comms.templates.save":
      track("communication_template_saved", {
        mode: input.id ? "updated" : "created",
        communication_kind: input.kind as never,
      });
      break;
    case "comms.sendDecision":
    case "comms.sendReminder":
    case "comms.sendConsolidatedDecision":
    case "comms.sendCrmCampaign":
      track("communication_sent", {
        communication_kind:
          operation === "comms.sendDecision"
            ? "decision"
            : operation === "comms.sendReminder"
              ? "reminder"
              : operation === "comms.sendConsolidatedDecision"
    ...
    case "tasks.create":
      track("task_created", {
        task_target: input.targetType as never,
        source: "manual",
      });
      break;
    case "tasks.setStatus":
      track("task_status_updated", { task_status: input.status as never });
      break;
    case "taskTemplates.remove":
      track("record_removed", { record_type: "task_template" });
      break;
    case "emailIntegrations.save":
      track("integration_connected", { integration: "email" });
      break;
    case "emailIntegrations.test":
      track("integration_tested", {
        integration: "email",
        outcome: "succeeded",
      });
      break;
    case "emailIntegrations.disconnect":
      track("integration_disconnected", { integration: "email" });
      break;
    case "agentProviderSettings.saveManaged":
    case "agentProviderSettings.saveByok":
      track("integration_connected", { integration: "ai_provider" });
      break;
    case "agentProviderSettings.disconnectByok":
      track("integration_disconnected", { integration: "ai_provider" });
      break;
    case "apiKeys.generate":
      track("api_key_created", {
        scope_count: Array.isArray(input.scopes) ? input.scopes.length : 1,
      });
      break;
    case "apiKeys.revoke":
      track("api_key_revoked", {});
      break;
    case "publicEmbeds.save":
      track("embed_saved", {
        mode: input.id ? "updated" : "created",
        enabled: typeof input.enabled === "boolean" ? input.enabled : undefined,
        embed_view: input.view as never,
      });
      break;
    case "publicEmbeds.duplicate":
      track("embed_duplicated", {});
      break;
    case "publicEmbeds.remove":
      track("embed_removed", {});
      break;
    case "portalForms.submit":
      track("portal_form_completed", {});
      break;
    case "availability.upsert":
      track("availability_updated", {
        unavailable_count: Array.isArray(input.unavailable)
          ? input.unavailable.length
          : 0,
      });
      break;
    case "organizers.completeOnboarding":
...
function workflowFor(
  operation: WriteOperation,
): AnalyticsEventProperties["workflow_failed"]["workflow"] | undefined {
  ...
  if (
    operation.startsWith("emailIntegrations.") ||
    operation.startsWith("agentProviderSettings.")
  )
    return "integration";
  if (
    operation === "organizers.completeOnboarding" ||
    operation.startsWith("organizations.")
  )
    return "onboarding";
  if (operation.startsWith("evaluations.")) return "review";
  if (operation.startsWith("speakers.")) return "speaker";
  if (operation.startsWith("sponsors.") || operation.startsWith("sponsor"))
    return "sponsor";
  if (
    operation.startsWith("submissions.") ||
    operation === "publicForms.submit"
  )
    return "submission";
  if (operation.startsWith("tasks.") || operation.startsWith("taskTemplates."))
    return "task";
  return undefined;
}
...
export function createRepository(transport: DataTransport): Repository {
    ...
    write: async <Result>(
      ...
      try {
        ...
        if (operation === "publicForms.submit")
          track("public_submission_failed", {
            error_category: analyticsErrorCategory(error),
          });
        else if (operation === "emailIntegrations.test")
          track("integration_tested", {
            integration: "email",
            outcome: "failed",
          });
          ...
          operation === "comms.sendReminder" ||
          operation === "comms.sendConsolidatedDecision" ||
          operation === "comms.sendCrmCampaign"
        )
          track("communication_failed", {
            communication_kind:
              operation === "comms.sendDecision"
                ? "decision"
                : operation === "comms.sendReminder"
  ...
  return {
    activity: {
      list: ({ eventId }) =>
        transport.read<ActivityEntry[]>("activity.list", { eventId }),
    },
    analytics: {
      summary: ({ eventId }) =>
    ...
    crm: {
      ...
      listSegments: ({ eventId }) =>
        transport.read<CrmSegment[]>("crm.segments.list", { eventId }),
      save: (input) => transport.write("crm.save", input),
      saveSegment: (input) => transport.write("crm.segments.save", input),
      assignToEvent: (input) => transport.write("crm.assignToEvent", input),
      unlinkFromEvent: (input) => transport.write("crm.unlinkFromEvent", input),
      archive: (input) => transport.write("crm.archive", input),
      bulkStage: (input) => transport.write("crm.bulkStage", input),
      backfillEvent: ({ eventId }) =>
        transport.write("crm.backfillEvent", { eventId }),
      listSources: ({ eventId }) =>
        transport.read<CrmSource[]>("crm.sources.list", { eventId }),
      connectSource: (input) => transport.write("crm.sources.connect", input),
      startSourceOAuth: (input) =>
        transport.write("crm.sources.startOAuth", input),
      finishSourceOAuth: (input) =>
        transport.write("crm.sources.finishOAuth", input),
      syncSource: (input) => transport.write("crm.sources.sync", input),
    },
    agentProviderSettings: {
      status: ({ eventId }) =>
        transport.read<AgentProviderSetting>("agentProviderSettings.status", {
          eventId,
        }),
      saveManaged: ({ eventId }) =>
        transport.write("agentProviderSettings.saveManaged", { eventId }),
      saveByok: (input) =>
        transport.write("agentProviderSettings.saveByok", input),
      disconnectByok: ({ eventId }) =>
        transport.write("agentProviderSettings.disconnectByok", { eventId }),
      assignBillingOwner: ({ eventId }) =>
        transport.write("agentProviderSettings.assignBillingOwner", {
          eventId,
        }),
    },
    agentRuns: {
      canUse: ({ eventId }) =>
        transport.read<boolean>("agentRuns.canUse", { eventId }),
      suggestions: ({ eventId }) =>
        transport.read<AgentSuggestion[]>("agentRuns.suggestions", { eventId }),
      list: ({ eventId, limit }) =>
        transport.read<AgentRun[]>("agentRuns.list", { eventId, limit }),
      get: ({ eventId, runId }) =>
        transport.read<AgentRunDetail | null>("agentRuns.get", {
          eventId,
          runId,
        }),
      ...
      approveTaskProposal: (input) =>
        transport.write<{ createdTaskIds: string[] }>(
          "agentRuns.approveTaskProposal",
          input,
        ),
      approveMessageProposal: (input) =>
        transport.write<{ createdDraftIds: string[] }>(
          "agentRuns.approveMessageProposal",
          input,
        ),
      rejectProposal: (input) =>
        transport.write<void>("agentRuns.rejectProposal", input),
    },
    apiKeys: {
      list: ({ eventId }) =>
        transport.read<ApiKey[]>("apiKeys.list", { eventId }),
      generate: ({ eventId, label, scopes = ["events:read"] }) =>
        transport.write<GeneratedApiKey>("apiKeys.generate", {
          eventId,
          label,
          scopes,
        }),
      revoke: ({ eventId, id }) =>
        transport.write<void>("apiKeys.revoke", { eventId, id }),
      auditLog: ({ eventId }) =>
        transport.read<ApiAuditLogEntry[]>("apiKeys.auditLog", { eventId }),
    },
    emailIntegrations: {
      status: ({ eventId }) =>
    ...
    contentIntegrations: {
      status: ({ eventId, provider }) =>
        transport.read<ContentIntegration | null>(
          "contentIntegrations.status",
          {
            eventId,
            provider,
      ...
      connectNotion: (input: NotionConnectInput) =>
        transport.write<{ status: "connected" }>(
          "contentIntegrations.connectNotion",
          input,
        ),
      importNotion: ({ eventId }) =>
        transport.write<NotionImportResult>(
          "contentIntegrations.importNotion",
          { eventId },
        ),
      ...
      startOAuth: (input) =>
        transport.write<{ url: string }>(
          "contentIntegrations.startOAuth",
          input,
        ),
      listNotionOAuthDatabases: (input) =>
        transport.write("contentIntegrations.listNotionOAuthDatabases", input),
      finishNotionOAuth: (input) =>
        transport.write("contentIntegrations.finishNotionOAuth", input),
      listAirtableOAuthBases: (input) =>
        transport.write("contentIntegrations.listAirtableOAuthBases", input),
      listAirtableOAuthTables: (input) =>
        transport.write("contentIntegrations.listAirtableOAuthTables", input),
      finishAirtableOAuth: (input) =>
        transport.write("contentIntegrations.finishAirtableOAuth", input),
      importAirtable: ({ eventId }) =>
        transport.write<AirtableImportResult>(
          "contentIntegrations.importAirtable",
          { eventId },
        ),
      ...
      publishSanity: ({ eventId }) =>
        transport.write<SanityPublishResult>(
          "contentIntegrations.publishSanity",
          { eventId },
        ),
      disconnect: ({ eventId, provider }) =>
        transport.write<{ status: "disconnected" }>(
          "contentIntegrations.disconnect",
          {
            eventId,
            provider,
    ...
    events: {
      ...
      portalSpeakerIdentity: () =>
        transport.read<PortalSpeakerIdentity>(
          "events.portalSpeakerIdentity",
          {},
        ),
    ...
    files: {
      generateUploadUrl: () =>
        transport.write<{ uploadUrl: string }>("files.generateUploadUrl", {}),
      getUrl: (storageId) =>
    ...
    forms: {
      ...
      saveField: (input) => {
        ...
        return transport.write<string>("forms.saveField", {
          id,
          ...(eventId ? { eventId } : {}),
          field,
        });
    ...
    speakerNotes: {
      list: ({ eventId, speakerId }) =>
        transport.read<SpeakerNote[]>("speakerNotes.list", {
          eventId,
          speakerId,
        }),
    ...
    agenda: {
      ...
      listForSpeaker: ({ eventId, speakerId }) =>
        transport.read<SpeakerAgendaItem[]>("agenda.listForSpeaker", {
          eventId,
          speakerId,
        }),
      ...
      checkPlacement: (input) =>
        transport.read<AgendaPlacementConflict[]>(
          "agenda.checkPlacement",
          input,
        ),
    ...
    taskTemplates: {
      ...
      ensureStarters: (input) =>
        transport.write<{ created: number }>(
          "taskTemplates.ensureStarters",
          input,
        ),
    ...
    comms: {
      ...
      listDrafts: ({ eventId }) =>
        transport.read<CommunicationDraft[]>("comms.listDrafts", { eventId }),
      ...
      sendConsolidatedDecision: (input) =>
        transport.write<CommSendResult>(
          "comms.sendConsolidatedDecision",
          input,
        ),
    ...
    publicEmbeds: {
      list: ({ eventId }) =>
        transport.read<Embed[]>("publicEmbeds.list", { eventId }),
      getAdmin: (input) =>
        transport.read<Embed | null>("publicEmbeds.getAdmin", input),
      preview: (input) =>
        transport.read<PublicEmbedView | null>("publicEmbeds.preview", input),
      save: (input) => transport.write<EmbedId>("publicEmbeds.save", input),
      duplicate: (input) =>
        transport.write<EmbedId>("publicEmbeds.duplicate", input),
      remove: async (input) => {
        await transport.write<null>("publicEmbeds.remove", input);
      },
      getPublic: (embedId) =>
    ...
    portalResources: {
      listAdmin: ({ eventId }) =>
        transport.read<PortalResourcePage[]>("portalResources.listAdmin", {
          eventId,
        }),
      listPublished: ({ eventId, speakerId }) =>
        transport.read<PortalResourcePage[]>("portalResources.listPublished", {
          eventId,
          speakerId,
        }),
      save: (input) => transport.write<string>("portalResources.save", input),
      reorder: (input) =>
        transport.write<void>("portalResources.reorder", input),
