...
const convexActions = new Set<WriteOperation>(["emailIntegrations.save", "emailIntegrations.test", "emailIntegrations.disconnect", "evaluations.sendReviewerReminders", "apiKeys.generate", "comms.sendDecision", "comms.sendReminder", "comms.sendConsolidatedDecision"]);
...
export function normalize(operation: ReadOperation | WriteOperation, value: unknown): unknown {
  ...
  if (operation === "events.get" || operation === "events.getBySlug" || operation === "emailIntegrations.status" || operation === "organizers.getMine") return row(value);
