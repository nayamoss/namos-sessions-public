...
export const convexFunction: Record<ReadOperation | WriteOperation, string> = {
  ...
  "apiKeys.list": "apiKeys:list", "apiKeys.generate": "apiKeysActions:generate", "apiKeys.revoke": "apiKeys:revoke",
  ...
  "agentProviderSettings.status": "agentProviderSettings:status", "agentProviderSettings.saveManaged": "agentProviderSettingsActions:saveManaged", "agentProviderSettings.saveByok": "agentProviderSettingsActions:saveByok",
};
...
const convexActions = new Set<WriteOperation>(["eventMembers.invite", "eventMembers.resend", "eventMembers.remove", "emailIntegrations.save", "emailIntegrations.test", "emailIntegrations.disconnect", "agentProviderSettings.saveManaged", "agentProviderSettings.saveByok", "evaluations.sendReviewerReminders", "apiKeys.generate", "comms.sendDecision", "comms.sendReminder", "comms.sendConsolidatedDecision"]);
...
export function normalize(operation: ReadOperation | WriteOperation, value: unknown): unknown {
  ...
  if (operation === "events.get" || operation === "events.getBySlug" || operation === "emailIntegrations.status" || operation === "organizers.getMine" || operation === "profiles.getMine") return row(value);
  ...
  if (operation === "events.rooms.list" || operation === "events.tracks.list" || operation === "eventMembers.list" || operation === "tags.list" || operation === "speakers.documents.list" || operation === "agenda.list" || operation === "agenda.listForSpeaker" || operation === "evaluations.list" || operation === "evaluations.plans.list" || operation === "evaluations.assignments.list" || operation === "tasks.list" || operation === "taskTemplates.list" || operation === "availability.list" || operation === "organizers.list" || operation === "apiKeys.list") return documentRows(value).map(row);
...
export function normalizeInput(operation: ReadOperation | WriteOperation, input: Record<string, unknown>) {
  if (operation === "events.save" && "id" in input) {
    // `event` here is a previously-read row, so it still carries server-managed timestamps
    // (`createdAt`/`updatedAt`) that `convex/events.ts`'s `save` mutation doesn't accept as
    // args — it sets them itself. Strip them the same way `row()` already strips `_creationTime`.
    const { id, createdAt: _createdAt, updatedAt: _updatedAt, ...event } = input;
...
function createConvexTransport(getToken?: TokenGetter): DataTransport {
  ...
  return {
    ...
    write: async (operation, input) => {
      ...
      const name = convexFunction[operation] as never;
