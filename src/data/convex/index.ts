...
export const convexFunction: Record<Exclude<ReadOperation | WriteOperation, "publicForms.submit">, string> = {
  ...
  "agentRuns.canUse": "agentRuns:canUse", "agentRuns.suggestions": "agentRuns:suggestions", "agentRuns.list": "agentRuns:list", "agentRuns.get": "agentRuns:get", "agentRuns.create": "agentRuns:create", "agentRuns.respond": "agentRuns:respond", "agentRuns.retry": "agentRuns:retry", "agentRuns.cancel": "agentRuns:cancel", "agentRuns.approveTaskProposal": "agentRuns:approveTaskProposal", "agentRuns.approveMessageProposal": "agentRuns:approveMessageProposal", "agentRuns.rejectProposal": "agentRuns:rejectProposal",
...
export function normalize(operation: ReadOperation | WriteOperation, value: unknown): unknown {
  ...
  if (operation === "events.rooms.list" || operation === "events.tracks.list" || operation === "eventMembers.list" || operation === "tags.list" || operation === "speakers.documents.list" || operation === "speakerNotes.list" || operation === "agenda.list" || operation === "agenda.listForSpeaker" || operation === "evaluations.list" || operation === "evaluations.plans.list" || operation === "evaluations.assignments.list" || operation === "tasks.list" || operation === "taskTemplates.list" || operation === "availability.list" || operation === "organizers.list" || operation === "organizations.listMine" || operation === "apiKeys.list" || operation === "apiKeys.auditLog" || operation === "portalResources.listAdmin" || operation === "portalResources.listPublished") return documentRows(value).map(row);
