...
export function createAirtableTransport(getToken: TokenGetter = async () => null): DataTransport {
  const request = async <Result>(operation: ReadOperation | WriteOperation, input: Record<string, unknown>) => {
    ...
    if (operation.startsWith("organizers.") || operation.startsWith("eventMembers.") || operation === "events.listMine" || operation === "events.duplicate" || operation === "speakers.bulkImport") throw new Error("Airtable does not yet provide the organizer/RBAC boundary.");
