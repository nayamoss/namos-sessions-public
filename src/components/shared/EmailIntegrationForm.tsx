...
export function EmailIntegrationForm({ eventId }: { eventId: EventId }) {
  const repo = useRepo();
  ...
  const load = useCallback(async () => { setLoading(true); try { const current = await repo.emailIntegrations.status({ eventId }); setIntegration(current); if (current) { setAuthMethod(current.authMethod); setSender(current.sender); setRegion(current.region ?? ""); } } catch (cause) { setError(friendlyErrorMessage(cause, "Could not load email delivery settings.")); } finally { setLoading(false); } }, [eventId, repo]);
