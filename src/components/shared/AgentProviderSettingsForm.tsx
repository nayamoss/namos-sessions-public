...
export function AgentProviderSettingsForm({ eventId, onSaved }: { eventId: EventId; onSaved?: () => void }) {
  ...
  const assignBillingOwner = async () => {
    setError(undefined); setSuccess(undefined); setBusy(true);
    try {
      ...
      await load(); onSaved?.();
    } catch (cause) { setError(friendlyErrorMessage(cause, "Could not assign a billing owner.")); }
    finally { setBusy(false); }
  };
  const disconnectByok = async () => {
    ...
    try {
      ...
      setSuccess("The organizer key was removed. New runs will use Namos-managed AI.");
      await load(); onSaved?.();
    } catch (cause) { setError(friendlyErrorMessage(cause, "Could not disconnect the organizer key.")); }
    finally { setBusy(false); }
  };
  ...
  return <div className="space-y-6">
    ...
    <section className="space-y-3 rounded-lg bg-background p-6">
      <div><h2 className="text-base font-semibold">Namos-managed AI usage</h2><p className="mt-1 text-sm text-muted-foreground">Namos-managed runs use the event creator’s Clerk Billing plan. Organizer-provided keys are never charged by Namos.</p></div>
      {!setting?.billingOwnerAssigned ? <div className="flex flex-wrap items-center justify-between gap-3 rounded-md bg-muted/60 p-4"><p className="text-sm text-muted-foreground">This existing event has no billing owner. An installation owner must assign one before managed AI can be used.</p><Button type="button" variant="outline" onClick={() => void assignBillingOwner()} disabled={busy}>Assign me as billing owner</Button></div> : setting?.managedUsage ? <dl className="grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-muted-foreground">Monthly runs</dt><dd>{setting.managedUsage.usedRuns + setting.managedUsage.reservedRuns} of {setting.managedUsage.runLimit}</dd></div><div><dt className="text-muted-foreground">Monthly tokens</dt><dd>{(setting.managedUsage.usedTokens + setting.managedUsage.reservedTokens).toLocaleString()} of {setting.managedUsage.tokenLimit.toLocaleString()}</dd></div><div><dt className="text-muted-foreground">Plan</dt><dd>{setting.managedUsage.planSlug}</dd></div></dl> : <p className="text-sm text-muted-foreground">Your plan allowance will appear after the first managed run is reserved.</p>}
    </section>
    <section className="space-y-5 rounded-lg bg-background p-6">
      ...
      <div className="flex flex-wrap gap-3"><Button type="button" variant="accent" onClick={() => void save()} disabled={busy || (mode === "managed" && (!setting?.managedAvailable || !setting.billingOwnerAssigned))}>{busy ? "Saving…" : "Save AI provider"}</Button>{setting?.mode === "bring_your_own" && <Button type="button" variant="outline" onClick={() => void disconnectByok()} disabled={busy || !setting.managedAvailable || !setting.billingOwnerAssigned}>Disconnect organizer key</Button>}</div>
    </section>
  </div>;
}
