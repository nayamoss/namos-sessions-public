...
export function ReadinessCategoryCard({ label, icon: Icon, items, notDateSpecificCount = 0, loadError }: { label: string; icon: LucideIcon; items: ReadinessItem[]; notDateSpecificCount?: number; loadError?: string }) {
  return <Card variant="muted" className="p-5" aria-label={label}>
    ...
    {items.length ? <ul className="mt-3 space-y-1">{items.map(item => <li key={item.id}><Link to={item.to} className="block rounded-md px-2 py-2.5 outline-none hover:bg-background/70"><p className="text-sm font-medium">{item.title}</p>{item.detail && <p className="mt-0.5 text-sm text-muted-foreground">{item.detail}</p>}</Link></li>)}</ul> : !loadError && <p className="mt-3 flex items-center gap-2 text-sm text-muted-foreground"><CheckCircle2 className="h-4 w-4" aria-hidden="true" />Nothing outstanding here.</p>}
