    ...
    >
      {min === 0 && (
        <Button
          ...
          aria-label={`${label}: 0 of ${max}`}
          disabled={disabled || readOnly}
          onClick={() => choose(0)}
          onKeyDown={onKeyDown}
          className={cn(
            "h-auto w-auto rounded-[4px] p-0.5 text-xs",
        ...
        {Array.from({ length: max }, (_, index) => {
          ...
          return (
            <Button
              ...
              className={cn(
                "h-auto w-auto rounded-[4px] p-0.5 hover:bg-transparent",
      ...
      <span className="ml-1 whitespace-nowrap text-xs text-muted-foreground">
        {value === undefined ? "Not scored" : `${value} / ${max}`}
      </span>
    </div>
  );
}
