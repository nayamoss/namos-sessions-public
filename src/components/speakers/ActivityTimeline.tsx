...
type TimelineType =
  "all" | "email" | "note" | "task" | "meeting" | "document" | "status";
...
const formatDateHeader = (value: number) =>
  new Date(value).toLocaleDateString(undefined, {
    month: "short",
    day: "numeric",
    year: "numeric",
  });
const labels: Record<SpeakerTimelineEvent["type"], string> = {
  email: "Email",
  note: "Note",
  task: "Task",
  meeting: "Meeting",
...
export function ActivityTimeline({
  ...
  const counts = useMemo(
    () =>
      events.reduce<Record<TimelineType, number>>(
        (next, event) => ({
          ...next,
          all: next.all + 1,
          [event.type]: next[event.type] + 1,
        }),
        {
          all: 0,
          email: 0,
      ...
      {loading ? (
        ...
        <div className="rounded-lg bg-background px-3 py-6 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Add a note or create a task to build the speaker record.
          </p>
        </div>
      ) : (
            ...
            <section key={group.date} className="space-y-2">
              ...
              <div className="space-y-2">
                {group.items.map((event) => (
                  <article
                    key={event.id}
                    onClick={compact ? () => onOpenEvent?.(event) : undefined}
                    className={`group rounded-lg bg-background p-3 ${compact && onOpenEvent ? "cursor-pointer hover:bg-muted" : ""}`}
                  >
                    <div className="flex items-start gap-2">
                      ...
                      {!compact && event.body ? (
                        <Button
                          ...
                          className="h-6 w-6 rounded p-1 text-muted-foreground opacity-0 transition-opacity hover:text-foreground group-hover:opacity-100 focus-visible:opacity-100"
                          title="Copy activity"
                        >
                          <Clipboard className="h-3.5 w-3.5" />
                          <span className="sr-only">Copy activity</span>
                        </Button>
                      ) : null}
                    </div>
                  </article>
