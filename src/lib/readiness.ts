...
function agendaConflictItem(conflict: AgendaConflict, agendaById: Map<string, AgendaItem>, event: Event): ReadinessItem | undefined {
  ...
  return { id: `${conflict.reason}-${conflict.itemA}-${conflict.itemB}`, title, detail: `Open ${first.title} to resolve this conflict.`, to: `/events/${event.slug}/program/agenda/${encodeURIComponent(first.id)}/edit`, eventDate: dayFor(first.startTime, event) };
}
...
export function projectReadinessGroups({ event, agenda, agendaConflicts, speakerRows, submissions, tasks, comms, now }: { event: Event; agenda: AgendaItem[]; agendaConflicts: AgendaConflict[]; speakerRows: SpeakerOperationsRow[]; submissions: Submission[]; tasks: OnboardingTask[]; comms: Comm[]; now: number }): ReadinessGroup[] {
  const eventBase = `/events/${event.slug}`;
  ...
  const speakerItems = speakerRows.filter(row => row.confirmationStatus !== "confirmed" && row.submissions.length > 0).map(row => ({ id: row.id, title: `${row.name} has not confirmed`, detail: row.submissions.map(submission => submission.title).join(", "), to: `${eventBase}/program/speakers/${encodeURIComponent(row.id)}/edit`, eventDate: earliestAgendaDayBySpeaker.get(row.id) }));
  const taskItems = tasks.filter(task => task.status !== "completed" && task.dueDate !== undefined && task.dueDate < now).map(task => ({ id: task.id, title: `Overdue: ${task.title}`, detail: `Due ${new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric" }).format(task.dueDate)}`, to: `${eventBase}/portals/tasks/${encodeURIComponent(task.id)}/edit`, eventDate: utcCalendarDate(task.dueDate) }));
  const proposalItems = submissions.filter(submission => undecided.has(submission.status)).map(submission => ({ id: submission.id, title: `Decision needed: ${nameForSubmission(submission)}`, detail: submission.status === "pending" ? "Pending review" : submission.status === "accept_queue" ? "In accept queue" : submission.status === "maybe" ? "On hold" : "In decline queue", to: `${eventBase}/program/abstracts/${encodeURIComponent(submission.id)}/edit` }));
  const commItems = comms.filter(comm => comm.status === "failed").map(comm => ({ id: comm.id, title: `Delivery failed: ${comm.type || "Communication"}`, detail: comm.sentAt ?? comm.createdAt ? `Attempted ${new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(comm.sentAt ?? comm.createdAt!)}` : undefined, to: `${eventBase}/program/communications?selected=${encodeURIComponent(comm.id)}` }));
